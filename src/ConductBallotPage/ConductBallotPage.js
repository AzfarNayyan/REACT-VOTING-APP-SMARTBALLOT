    ...
    const handleVote = async () => {
        try {
            if (!selectedCandidate) {
                setValidationMessage('Please select a candidate to vote.');
                return;
            }

            // Check if voter has already cast a vote
            const responseCheckVote = await fetch('http://localhost:5501/checkVoteExistence', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            const dataCheckVote = await responseCheckVote.json();

            if (dataCheckVote.exists) {
                setValidationMessage('You have already cast your vote.');
                return;
            }

            // Validate voter credentials
            const responseValidateVoter = await fetch('http://localhost:5501/validateVoter', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            const dataValidateVoter = await responseValidateVoter.json();

            if (dataValidateVoter.valid) {
                // Record vote
